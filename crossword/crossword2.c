...
int getInput(char[][16],FILE*);
void createBoard(char[][15]);
void printBoard(char[][15],FILE*);
...
void anagram(int,char[][16], int[][3],FILE*);
void showPuzzle(char[][15],char[][15],FILE*);
 ...
 */ 
  ...
  numberWords=getInput(words,ifp);
  ...
  for(i=0;i<10000;i++){
    ...
    if(success==1){
      printBoard(crossword,ofp);
      showPuzzle(crossword,puzzle,ofp);
      anagram(numberWords,words,place,ofp);
      break;
    } else {
	...
	printBoard(crossword,ofp);
        showPuzzle(crossword,puzzle,ofp);
        anagram(numberWords,words,place,ofp);
	fprintf(ofp,"I'm sorry, but the board could not be created with these words.\nPlease try again.\n");
      }
...
void printBoard(char crossword[][15],FILE *ofp){//prints board
  int i,j;
  fprintf(ofp,"\nSolution:\n");
  for(i=0;i<15;i++){
     for(j=0;j<15;j++){
	fprintf(ofp,"%c",crossword[i][j]);
     }
	fprintf(ofp,"\n");}
}

int getInput(char words[][16], FILE *ifp){
  //printf("Please enter a list of words:\n");
  //fill words array with words
  int n,i,j,isValidWord;
  char newWord[100];
  for(i=0;i<20;i++){
    isValidWord=0;//assumes it is not a valid word
    while(!isValidWord){
...
void anagram(int n,char words[][16],int place[][3],FILE *ofp){//creates anagrams for the different words and displays them and their coordinates on the board
  int i,j=0;
  int r;
  ...
  for(i=0;i<n;i++){
	...
	if(place[i][2]==0){
	  fprintf(ofp,"%2i,%2i Down   %s\n",place[i][0],place[i][1],jumble);
	} else {
	  fprintf(ofp,"%2i,%2i Across %s\n",place[i][0],place[i][1],jumble);
	} 
  }fprintf(ofp,"\n");
}

void showPuzzle(char crossword[][15], char puzzle[][15],FILE *ofp){
  int i,j;
  fprintf(ofp,"\nCrossword Puzzle:\n");
  for(i=0;i<15;i++){
    for(j=0;j<15;j++){
